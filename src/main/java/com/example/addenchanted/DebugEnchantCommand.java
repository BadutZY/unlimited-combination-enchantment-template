 ...
 * Usage: /debugenchant
 * FIXED untuk MC 1.20.5 - menggunakan DataComponentTypes dengan Enchantment.value()
 */
public class DebugEnchantCommand {
    ...
    private static int execute(CommandContext<ServerCommandSource> context) {
        try {
            ...
            ItemEnchantmentsComponent enchants = heldItem.getOrDefault(DataComponentTypes.ENCHANTMENTS, ItemEnchantmentsComponent.DEFAULT);
            if (!enchants.isEmpty()) {
                ...
                for (RegistryEntry<Enchantment> enchantEntry : enchants.getEnchantments()) {
                    ...
                    context.getSource().sendFeedback(() ->
                            Text.literal("§f  - §e" + finalName + " §7lvl §a" + finalLevel), false);
                }
            } else {
            ...
            if (heldItem.isOf(Items.ENCHANTED_BOOK)) {
                ItemEnchantmentsComponent storedEnchants = heldItem.getOrDefault(
